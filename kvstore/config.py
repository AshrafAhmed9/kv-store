"""Runtime settings, loaded from KV_* environment variables at boot.

Kept as one typed, validated object instead of scattered os.environ.get()
calls, so a bad setting (e.g. memtable_size=0) fails fast at startup instead
of causing a confusing bug three layers deep at runtime.
"""
...
def load() -> Config:
    """Build a Config from the environment, with sane defaults. Call once at boot."""
    return Config(
